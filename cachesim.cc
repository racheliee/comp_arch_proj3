...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
    ...
    size_t way = 0;
    size_t temp_way = 0;
    uint64_t oldest_time = 0; // should be the smallest time stamp

    for (size_t i = 0; i < ways; i++)
    {
        temp_way = idx * ways + i;
        if (time_stamp[temp_way] < oldest_time)
        {
            oldest_time = time_stamp[temp_way];
            way = i;
        }
    }

    // update current time and assign it to time_stamp
    curr_time++;
    time_stamp[idx * ways + way] = curr_time;
    // change end
...
uint64_t *fa_cache_sim_t::check_tag(uint64_t addr)
{
    ...
    if (it == tags.end()){
        ...
        // update current time and assign it to time_stamp
        curr_time++;
        time_stamp[it->second] = curr_time;
        return &it->second;
    }
    // change end
}
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
    ...
    for(size_t i = 0; i < ways; i ++){
        if(time_stamp[i] < oldest_time){
        oldest_time = time_stamp[i];
        old_tag = i;
        }
    }
